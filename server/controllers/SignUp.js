...
//here we are only hashing the password using bcrypt no jwt
...
const validateSignUpData = async (req, res) => {
  ...
  if (password.trim().length === 0) {
    ...
    res
      .status(400)
      .json({ message: "Minimum length of the password should be 6" });
    return false;
  }

  // Checking if user already exists (Validation function)
  //exec(): This is a method used to execute the query.
